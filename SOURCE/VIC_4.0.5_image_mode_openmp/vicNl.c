...
**********************************************************************/
{
  ...
  if (options.GRID_INPUT) nldas_forcing = read_once_nldas_forcing(&filenames, global_param);

  /** Gridded Output: initialize **/
  if (options.GRID_OUTPUT) write_data_nldas_grid((out_data_struct *)NULL, &filenames, &global_param, -9999, global_param.nrecs, -1);
  
  /** read in all soil params for all cells at onece **/
  
  soil_con_all = (soil_con_struct *) calloc(NCELLS, sizeof(soil_con_struct));
  veg_con_all = (veg_con_struct **) calloc(NCELLS, sizeof(veg_con_struct*));
  ...
  while ((fscanf(infiles.soilparam, "%d", &flag))!=EOF) {
      if (flag) {
          ...
          veg_con_all[cell_cnt] = read_vegparam(infiles.vegparam, soil_con.gridcel,Nveg_type);
          cell_cnt++;
      }
 ...
 for (cell_cnt=1; cell_cnt<=NCELLS; cell_cnt++) {
  ...
  if (options.GRID_OUTPUT) write_data_nldas_grid((out_data_struct *)NULL, &filenames, &global_param, -9999, global_param.nrecs, -2);
