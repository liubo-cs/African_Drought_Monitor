...
**********************************************************************/
{
  ...
  options.INPUT_GRID_NX = 0;
  options.INPUT_GRID_NY = 0;
  options.INPUT_GRID_XO = MISSING;
  options.INPUT_GRID_YO = MISSING;
  options.INPUT_GRID_DX = MISSING;
  options.INPUT_GRID_DX = MISSING;
  options.OUTPUT_GRID_NX = 0;
  options.OUTPUT_GRID_NY = 0;
  options.OUTPUT_GRID_XO = MISSING;
  options.OUTPUT_GRID_YO = MISSING;
  options.OUTPUT_GRID_DX = MISSING;
  options.OUTPUT_GRID_DX = MISSING;
  ...
  while(!feof(gp)) {
	...
	****************************/
      ...
      /** Gridded Input/Output **/
      else if(strcasecmp("GRID_INPUT",optstr)==0) {
        sscanf(cmdstr,"%*s %s",flgstr);
        if(strcasecmp("TRUE",flgstr)==0) options.GRID_INPUT = TRUE;
        else options.GRID_INPUT = FALSE;
      }

      else if(strcasecmp("GRID_OUTPUT",optstr)==0) {
        sscanf(cmdstr,"%*s %s",flgstr);
        if(strcasecmp("TRUE",flgstr)==0) options.GRID_OUTPUT = TRUE;
        else options.GRID_OUTPUT = FALSE;
      }
      ...
      else if(strcasecmp("OUTPUT_PER_STEP",optstr)==0) {
        sscanf(cmdstr,"%*s %s",flgstr);
        if(strcasecmp("TRUE",flgstr)==0) options.OUTPUT_PER_STEP = TRUE;
        else options.OUTPUT_PER_STEP = FALSE;
      }
      
      /** Obtain Input/Output Grid Definitions **/
      else if(strcasecmp("INPUT_GRID_DEF",optstr)==0) {
        sscanf(cmdstr,"%*s %d %d %lf %lf %lf %lf", &options.INPUT_GRID_NX, &options.INPUT_GRID_NY, 
                                                   &options.INPUT_GRID_XO, &options.INPUT_GRID_YO, 
                                                   &options.INPUT_GRID_DX, &options.INPUT_GRID_DY);
      }

      else if(strcasecmp("OUTPUT_GRID_DEF",optstr)==0) {
        sscanf(cmdstr,"%*s %d %d %lf %lf %lf %lf", &options.OUTPUT_GRID_NX, &options.OUTPUT_GRID_NY, 
                                                   &options.OUTPUT_GRID_XO, &options.OUTPUT_GRID_YO, 
                                                   &options.OUTPUT_GRID_DX, &options.OUTPUT_GRID_DY);
      }
  ...
  *********************************/
  ...
  /* Check grid input/output paramteres */
  
  if (options.GRID_INPUT) {
      if (options.INPUT_GRID_NX == 0) {
          fprintf(stderr, "INPUT_GRID_NX not provided, set to NLDAS default value %d.\n", NLDAS_NCOLS);
          options.INPUT_GRID_NX = NLDAS_NCOLS;
      }
      if (options.INPUT_GRID_NY == 0) {
          fprintf(stderr, "INPUT_GRID_NY not provided, set to NLDAS default value %d.\n", NLDAS_NROWS);
          options.INPUT_GRID_NY = NLDAS_NROWS;
      }
      if (options.INPUT_GRID_XO == MISSING) {
          fprintf(stderr, "INPUT_GRID_XO not provided, set to NLDAS default value %lf.\n", NLDAS_LL_LON);
          options.INPUT_GRID_XO = NLDAS_LL_LON;
      }
      if (options.INPUT_GRID_YO == MISSING) {
          fprintf(stderr, "INPUT_GRID_YO not provided, set to NLDAS default value %lf.\n", NLDAS_LL_LAT);
          options.INPUT_GRID_YO = NLDAS_LL_LAT;
      }
      if (options.INPUT_GRID_DX == MISSING) {
          fprintf(stderr, "INPUT_GRID_DX not provided, set to NLDAS default value %lf.\n", NLDAS_CELL);
          options.INPUT_GRID_DX = NLDAS_CELL;
      }
      if (options.INPUT_GRID_DY == MISSING) {
          fprintf(stderr, "INPUT_GRID_DY not provided, set to NLDAS default value %lf.\n", NLDAS_CELL);
          options.INPUT_GRID_DY = NLDAS_CELL;
      }
  }
  
  if (options.GRID_OUTPUT) {
      if (options.OUTPUT_GRID_NX == 0) {
          fprintf(stderr, "OUTPUT_GRID_NX not provided, set to NLDAS default value %d.\n", NLDAS_NCOLS);
          options.OUTPUT_GRID_NX = NLDAS_NCOLS;
      }
      if (options.OUTPUT_GRID_NY == 0) {
          fprintf(stderr, "OUTPUT_GRID_NY not provided, set to NLDAS default value %d.\n", NLDAS_NROWS);
          options.OUTPUT_GRID_NY = NLDAS_NROWS;
      }
      if (options.OUTPUT_GRID_XO == MISSING) {
          fprintf(stderr, "OUTPUT_GRID_XO not provided, set to NLDAS default value %lf.\n", NLDAS_LL_LON);
          options.OUTPUT_GRID_XO = NLDAS_LL_LON;
      }
      if (options.OUTPUT_GRID_YO == MISSING) {
          fprintf(stderr, "OUTPUT_GRID_YO not provided, set to NLDAS default value %lf.\n", NLDAS_LL_LAT);
          options.OUTPUT_GRID_YO = NLDAS_LL_LAT;
      }
      if (options.OUTPUT_GRID_DX == MISSING) {
          fprintf(stderr, "OUTPUT_GRID_DX not provided, set to NLDAS default value %lf.\n", NLDAS_CELL);
          options.OUTPUT_GRID_DX = NLDAS_CELL;
      }
      if (options.OUTPUT_GRID_DY == MISSING) {
          fprintf(stderr, "OUTPUT_GRID_DY not provided, set to NLDAS default value %lf.\n", NLDAS_CELL);
          options.OUTPUT_GRID_DY = NLDAS_CELL;
      }
  }
